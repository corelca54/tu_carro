...
import { Send, Upload, MapPin, Gauge } from "lucide-react"; 
...
interface VehiculoAnuncio {
    ...
    precio: number;
    ciudad: string;
    publicado_en: string; // Mantenemos este nombre para la interfaz
    imagen_url: string; 
}
...
export default function VenderPage() {
    ...
    // FUNCIÓN PARA CARGAR LOS VEHÍCULOS DEL USUARIO ACTUAL (CORREGIDA)
    const fetchMisVehiculos = async (userId: string) => {
        const { data, error } = await supabase
            .from("vehiculos")
            // CORRECCIÓN 1: Cambiamos 'publicado_en' por 'creado_en' en el SELECT
            .select(`id, marca, modelo, año, precio, ciudad, creado_en, imagenes`)
            .eq("usuario_id", userId) 
            // CORRECCIÓN 2: Cambiamos 'publicado_en' por 'creado_en' en el ORDER
        ...
        if (error) {
            ...
            const formattedData: VehiculoAnuncio[] = data.map((v: any) => ({
                ...
                precio: v.precio,
                ciudad: v.ciudad,
                // CORRECCIÓN 3: Mapeamos 'creado_en' al campo de la interfaz 'publicado_en'
                publicado_en: v.creado_en, 
                imagen_url: v.imagenes && v.imagenes.length > 0 ? v.imagenes[0] : '/placeholder_car.jpg',
            }));
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            let imageUrl = '';
            if (imagenFile) {
                imageUrl = await uploadImage(user.id, imagenFile);
            } else {
                 throw new Error("Debes seleccionar una imagen para publicar.");
            }

            // 2. Insertar los datos en la tabla 'vehiculos'
                ...
                .insert([
                    {
                        ...
                        ciudad: ciudad,
                        imagenes: imageUrl ? [imageUrl] : [], // Guarda la URL pública
                    }
                ]);
            ...
            setDescripcion(""); setCiudad(""); setImagenFile(null);
            fetchMisVehiculos(user.id); 

        } catch (error: any) {
            setMensaje("❌ Error en la publicación: " + error.message);
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto mt-10 p-6 space-y-10 bg-white rounded-xl shadow-2xl">
            <h1 className="text-3xl font-bold text-center text-blue-800">
                Publicar Nuevo Vehículo
            </h1>
            
            {mensaje && (
                <p className={`text-center font-medium p-3 rounded-lg ${mensaje.startsWith("✅") ? 'bg-green-100 text-green-700' : 'text-red-100 text-red-700'}`}>
                    {mensaje}
                </p>
            )}

            {/* FORMULARIO DE PUBLICACIÓN (CRUD: CREATE) */}
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-xl border">
                ...
                <input type="number" placeholder="Kilometraje (km)" value={kilometraje} onChange={(e) => setKilometraje(e.target.value)} required className="border p-3 rounded-lg focus:border-yellow-500" />
                ...
                {/* Campo de Subida de Imagen (Fotografía) */}
                <div className="relative border p-3 rounded-lg flex items-center justify-between">
                    <label className="text-gray-500 flex items-center">
                        <Upload className="w-5 h-5 mr-2" />
                        {imagenFile ? imagenFile.name : "Subir Foto Principal *"}
                    </label>
                    <input 
                        type="file" 
                        accept="image/*" 
                ...
                <button type="submit" disabled={isSubmitting} className="col-span-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition flex items-center justify-center disabled:bg-gray-400">
                    <Send className="w-5 h-5 mr-2" />
                    {isSubmitting ? "Publicando..." : "Publicar Carro"}
                </button>
            </form>

            {/* LISTADO DE MIS VEHÍCULOS (Funcional) */}
            <section>
                <h2 className="text-2xl font-bold text-center mb-6 text-gray-700">Mis Anuncios Publicados ({misVehiculos.length})</h2>
                ...
                {misVehiculos.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        {misVehiculos.map((v) => (
                            <div key={v.id} className="flex items-center p-4 bg-gray-50 border rounded-lg shadow-sm">
                                ...
                                <div className="flex-grow">
                                    ...
                                    <p className="text-xl font-bold text-green-600">${v.precio.toLocaleString('es-CO')}</p>
